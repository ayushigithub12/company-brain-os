...
import { fetchAllDriveFiles } from "@/lib/connectors/google-drive";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const connector = await prisma.connector.findFirst({
      where: { id: connectorId, workspaceId, type: "GOOGLE_DRIVE" },
    });
    if (!connector?.accessToken || !connector?.refreshToken) {
      return NextResponse.json({ success: false, error: "Google Drive not authenticated" }, { status: 400 });
    }

    await prisma.connector.update({ where: { id: connectorId }, data: { status: "SYNCING" } });

    const rawDocs = await fetchAllDriveFiles(connector.accessToken, connector.refreshToken, { maxFiles: 300 });
    ...
    if (rawDocs.length === 0) {
      ...
      return NextResponse.json({ success: true, data: { message: "No files found in Google Drive", indexed: 0, failed: 0, cardsExtracted: 0 } });
    }
    ...
    if (shouldExtract) {
      ...
      try {
        ...
        console.error("[Drive Sync] Extraction failed (non-fatal):", err);
      }
    }

    const message = indexed === 0
      ? "Everything is up to date — no changes found"
      : `${indexed} file${indexed !== 1 ? "s" : ""} updated${cardsExtracted > 0 ? `, ${cardsExtracted} new card${cardsExtracted !== 1 ? "s" : ""} extracted` : ""}`;
