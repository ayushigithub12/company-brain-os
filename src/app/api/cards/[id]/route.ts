...
// GET, PATCH /api/cards/[id] — fetch or update a single process card
...
export async function DELETE(
  ...
  try {
    ...
    const card = await prisma.processCard.findUnique({ where: { id: params.id }, select: { workspaceId: true } });
    if (!card) return NextResponse.json({ success: false, error: "Card not found" }, { status: 404 });

    const member = await prisma.workspaceMember.findUnique({
      where: { workspaceId_userId: { workspaceId: card.workspaceId, userId: session.user.id } },
    });
    if (!member || !["OWNER", "ADMIN"].includes(member.role)) {
      return NextResponse.json({ success: false, error: "Only owners and admins can delete cards" }, { status: 403 });
    }
